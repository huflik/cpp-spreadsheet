...
class CellExpr final : public Expr {
	public:
		...
		double Evaluate(const SheetInterface& sheet) const override {
			...
			Position* pos = const_cast<Position*>(cell_);
            const CellInterface* cell = sheet.GetCell(*pos);
			if (cell == nullptr) {
				SheetInterface& sheet_i = const_cast<SheetInterface&>(sheet);
				sheet_i.SetCell(*pos, ""s);
			}
			
			CellInterface::Value val = cell->GetValue();
			if (auto cell_val = std::get_if<double>(&val)) {
				return *cell_val;
			}

			if (auto cell_val = std::get_if<std::string>(&val)) {
				if (cell_val->empty()) {
					return 0.;
				}
				std::smatch sm;
                std::regex rex(R"(^\d+$)");
				std::regex_search(*cell_val, sm, rex);
			...
			throw *(std::get_if<FormulaError>(&val));
		}
